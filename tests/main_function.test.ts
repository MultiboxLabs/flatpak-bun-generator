...
describe("main function integration", () => {
  ...
  test("generates electron binary and node headers sources for npm electron dep", async () => {
    const testDir = join(TMP_DIR, "electron-npm");
    mkdirSync(testDir, { recursive: true });
    ...
    expect(x64Bin).toBeDefined();
    expect(x64Bin.url).toBe(
      "https://github.com/electron/electron/releases/download/v33.3.1/electron-v33.3.1-linux-x64.zip"
    );
    expect(x64Bin["dest-filename"]).toBe("electron-v33.3.1-linux-x64.zip");
    expect(x64Bin.sha256).toBeDefined();
    expect(x64Bin.sha256.length).toBe(64);
    ...
    expect(headersSources).toHaveLength(1);
    expect(headersSources[0].url).toBe(
      "https://artifacts.electronjs.org/headers/dist/v33.3.1/node-v33.3.1-headers.tar.gz"
    );
    expect(headersSources[0]["strip-components"]).toBe(1);
    expect(headersSources[0].sha256).toBeDefined();
    expect(headersSources[0].sha256.length).toBe(64);
  ...
  test("generates no electron sources when no electron dep exists", async () => {
    const testDir = join(TMP_DIR, "no-electron");
    mkdirSync(testDir, { recursive: true });
    ...
    expect(electronBinSources).toHaveLength(0);

    const headersSources = sources.filter(
      (s: any) => s.type === "archive" && s.dest === "electron-headers"
    );
    expect(headersSources).toHaveLength(0);
  });
});
