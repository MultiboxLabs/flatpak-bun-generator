...
import {
  ...
  detectElectronPackage,
  splitElectronVersion,
...
import type { ElectronInfo } from "../src/main.js";
...
describe("detectElectronPackage", () => {
  test("detects castlabs/electron-releases git dep", () => {
    const packagesMap: Record<string, any[]> = {
      "node-gyp": [
    ...
    const result = detectElectronPackage(packagesMap);
  ...
  test("detects electron/electron git dep", () => {
    const packagesMap: Record<string, any[]> = {
      "electron": [
        "electron@github:electron/electron#abc123",
        { "dependencies": { "@electron/get": "^2.0.0" }, "bin": { "electron": "cli.js" } },
        "electron-electron-abc123",
      ],
    };
    const result = detectElectronPackage(packagesMap);
    expect(result).not.toBeNull();
    expect(result!.type).toBe("git");
    if (result!.type === "git") {
      expect(result!.owner).toBe("electron");
      expect(result!.repo).toBe("electron");
      expect(result!.commit).toBe("abc123");
    }
  });

  test("detects npm electron dep", () => {
    const packagesMap: Record<string, any[]> = {
      "electron": [
        "electron@33.0.0",
        "",
    ...
    if (result!.type === "npm") {
      expect(result!.version).toBe("33.0.0");
    }
  });

  test("returns null when no electron key exists", () => {
    const packagesMap: Record<string, any[]> = {
      "node-gyp": [
        "node-gyp@github:electron/node-gyp#06b29aa",
        { "dependencies": {} },
  ...
  test("returns null for empty packages map", () => {
    expect(detectElectronPackage({})).toBeNull();
  });

  test("returns null for invalid electron entry", () => {
    const packagesMap: Record<string, any[]> = {
      "electron": [],
    };
    expect(detectElectronPackage(packagesMap)).toBeNull();
  });
});

describe("splitElectronVersion", () => {
  test("splits version with build metadata", () => {
    const result = splitElectronVersion("40.1.0+wvcus");
    expect(result.baseVersion).toBe("40.1.0");
    expect(result.buildMeta).toBe("wvcus");
  });

  test("splits version without build metadata", () => {
    const result = splitElectronVersion("40.1.0");
    expect(result.baseVersion).toBe("40.1.0");
    expect(result.buildMeta).toBeNull();
  });

  test("splits pre-release version with build metadata", () => {
    const result = splitElectronVersion("41.0.0-beta.5+wvcus");
    expect(result.baseVersion).toBe("41.0.0-beta.5");
    expect(result.buildMeta).toBe("wvcus");
  });
});
...
describe("parseElectronVersion", () => {
  test("parses castlabs version with +wvcus build metadata", () => {
    const info = parseElectronVersion("40.1.0+wvcus", "git", "castlabs", "electron-releases", "df5ab90");
  ...
  test("parses version without build metadata", () => {
    const info = parseElectronVersion("40.1.0", "git", "electron", "electron", "abc123");
  ...
  test("parses npm electron version", () => {
    const info = parseElectronVersion("33.0.0", "npm", "electron", "electron");
    expect(info.fullVersion).toBe("33.0.0");
    expect(info.baseVersion).toBe("33.0.0");
    expect(info.buildMeta).toBeNull();
    expect(info.source).toBe("npm");
    expect(info.owner).toBe("electron");
    expect(info.repo).toBe("electron");
    expect(info.commit).toBeUndefined();
  });

  test("parses pre-release version with build metadata", () => {
    const info = parseElectronVersion("41.0.0-beta.5+wvcus", "git", "castlabs", "electron-releases", "df5ab90");
...
describe("electronDownloadDirUrl", () => {
  ...
  test("builds npm electron download dir URL", () => {
    const info: ElectronInfo = {
      source: "npm",
      fullVersion: "33.0.0",
      baseVersion: "33.0.0",
    ...
    expect(electronDownloadDirUrl(info)).toBe(
      "https://github.com/electron/electron/releases/download/v33.0.0"
    );
  });
});
...
describe("electronBinaryUrl", () => {
  ...
  test("builds npm electron binary URL", () => {
    const info: ElectronInfo = {
      source: "npm",
      fullVersion: "33.0.0",
      baseVersion: "33.0.0",
    ...
    expect(electronBinaryUrl(info, "x64")).toBe(
      "https://github.com/electron/electron/releases/download/v33.0.0/electron-v33.0.0-linux-x64.zip"
    );
  });
});
...
describe("nodeHeadersUrl", () => {
  ...
  test("works for npm electron", () => {
    ...
    expect(nodeHeadersUrl(info)).toBe(
      "https://artifacts.electronjs.org/headers/dist/v33.0.0/node-v33.0.0-headers.tar.gz"
    );
  });
});
